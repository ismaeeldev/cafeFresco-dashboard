...
function BTitle({ title }) {
  return (
    <Grid>
      <Typography variant="h4" sx={{ fontWeight: 900, marginLeft: '10px' }}>
        {title}
      </Typography>
    </Grid>
  );
  ...
  if (!custom && main && main.type === 'collapse' && main.breadcrumbs === true) {
    breadcrumbContent = (
      <Card sx={card === false ? { mb: 3, bgcolor: 'transparent', ...sx } : { mb: 3, bgcolor: 'background.default', ...sx }} {...others}>
        <Box sx={{ p: 2, px: card === false ? 0 : 2 }}> {/* Increased padding from 1.25 to 2 */}
          <Grid
            container
          ...
          >
            ...
            <Grid>
              ...
              >
                <Typography component={Link} to="/admin" color="textSecondary" variant="h6" sx={linkSX}>
        ...
        {card === false && divider !== false && <Divider sx={{ mt: 2.5 }} />} {/* Increased margin-top from 2 to 2.5 */}
      </Card>
    );
  }
